...
def predict(pred_config):
    ...
    for words, slot_preds, intent_pred in zip(lines, slot_preds_list, intent_preds):
        ...
        for word, pred in zip(words, slot_preds):
            line = line + "{} ".format(pred)
        line_intent = intent_label_lst[intent_pred]
        line_slot = line.strip()
        l_result_slot.append(str(line_slot))
        l_result_intent.append(str(line_intent))
    dataframe_result["Results intent"] = l_result_intent
    dataframe_result["Results slot"] = l_result_slot
    dataframe_result.to_csv("results.csv", header=False, index = False)
